...
df <- select(df,-c("employstatus","race","lang","arrivalmonth","arrivalday","arrivalhour_bin","disposition"))
...
df$esi <- as.numeric(as.character(df$esi))
...
tree <- rpart(esi ~ ., data = df)

bst <- xgboost(
  data = data.matrix(subset(df, select = -esi)),
  label = df$esi, 
...
#save.image(file='myEnvironmentesi.RData')
